...
export class CarloadService {
  ...
  addCarLoad(carload: Partial<CarLoad>): Observable<CarLoad> {
    return this.http.post<CarLoad>(this.baseURL, carload).pipe(take(1));
  }

  updateCarLoad(id: string, carload: Partial<CarLoad>): Observable<CarLoad> {
    return this.http.put<CarLoad>(`${this.baseURL}/${id}`, carload).pipe(take(1));
  }

  markAsDelivered(id: string): Observable<CarLoad> {
    return this.http.put<CarLoad>(`${this.baseURL}/${id}/delivered`, {}).pipe(take(1));
  }
